  ...
  return data.value.categories
    .map((cat) => {
      ...
      if (cat.chips?.some((c) => c.toLowerCase().includes(q))) {
        return { ...cat, chips: cat.chips.filter((c) => c.toLowerCase().includes(q)), open: true }
      }
      if (cat.subcategories) {
        const subs = cat.subcategories
          .map((sub) => ({
            ...sub,
            chips: sub.chips.filter((c) => c.toLowerCase().includes(q) || sub.title.toLowerCase().includes(q)),
          }))
  ...
  <div class="catalogue-page">
    ...
    <template v-else-if="data">
      <header class="ph">
        <div class="wrap ph-inner">
          ...
          <div class="search-bar">
            <input v-model="query" type="text" placeholder="Search a profession — e.g. Electrician, Tax Consultant…" />
          </div>
          <div class="ph-stats">
          ...
          >
            <summary>
              <span class="summary-left">
                <span class="cat-hex" :style="{ background: colorMap[cat.color] || 'var(--green)' }"></span>
                {{ cat.name }}
            ...
            <div class="cat-body">
              ...
              <div v-else class="subcat" style="border-top:none;">
                <div class="chip-row">
                  <span v-for="chip in cat.chips" :key="chip" class="chip">{{ chip }}</span>
                </div>
                <div v-if="cat.note" class="market-flow-note" style="margin-top:14px;">{{ cat.note }}</div>
              </div>
            </div>
          </details>
        </div>
          ...
          <div class="profile-main">
            <div class="profile-grid">
              <div v-for="field in data.profile.fields" :key="field.label" class="pf-item">
                ...
                <div class="v" :class="field.tone">{{ field.value }}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="cta-strip">
        ...
        <div class="btns">
          <RouterLink :to="data.cta.primary.href" class="btn btn-fill">{{ data.cta.primary.label }}</RouterLink>
          <RouterLink :to="data.cta.secondary.href" class="btn btn-ghost">{{ data.cta.secondary.label }}</RouterLink>
        </div>
      </div>
    </template>
  </div>
